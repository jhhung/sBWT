...
void suffixsort(int *x, int *p, int n, int k, int l, std::function<bool(uint32_t, uint32_t)>& compare)
{
   int *pi, *pk;
   int i, j, s, sl;
      ...
      do {
         if ((s=*pi)<0) {
            ...
            //std::cerr<<"sorting "<<pi-I<<" to "<<pk-I<<std::endl;
            		tag=1;
                if (pk-pi > 10000)
                	std::sort ( pi, pk, compare );
                int g;

							  g=pk-I;                      /* group number.*/
							  V[*pi]=g;                    /* update group number of first position.*/
							  //if (pl==pm)
							  *pi=-1;
                //for (auto iter=pi; iter!=pk; iter++)
                //  update_group(iter, iter);
            }
            else
                sort_split(pi, pk-pi);
            pi=pk;              /* next group.*/
